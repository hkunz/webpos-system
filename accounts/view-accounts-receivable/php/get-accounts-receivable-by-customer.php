...
function customers_table_content($con) {
	...
	$table = "<table id='customer_table' class='common-table' cellspacing='0' cellpadding='0'><thead><tr><th>Customer</label></th><th width='150px' nowrap>Last Update</th><th nowrap>Grand Total</th><th nowrap>Payment</th></tr></thead><tbody>";
	while($row = $result->fetch_array()) {
		$customer = $row[$customer_field];
		$date = $row[$date_field];
		$payment = $row[$payment_field];
		$total = $row[$total_field];
		$table .= "<tr><td width='100%'>$customer</td><td nowrap>$date</td><td nowrap><span style='margin-right:2px;'>$currency</span>$total</td><td nowrap><span style='margin-right:2px;'$currency</span>$payment</td></tr>";
	}
	$table .= "</tbody></table>";
	return $table;
}
